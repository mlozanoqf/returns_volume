...
3. *You argue that volumes are used to calculate returns. This is not correct, the approach here is to find out whether volume is a factor that explains the stock returns. We use prices to calculate returns, not volume as you suggest. Then, you propose to use price instead of returns as a variable in your analysis. This is not correct because you are ignoring the main instructions of this repository. Here, we are interested in volume and returns, not prices. Even if we ignore the fact that you are using an incorrect approach, your current approach has several problems. For example, in some plots, you are using the same axis to plot prices and volume. This is not correct because volume is measured in the number of transactions and prices are in dollars. Using the same axis in these plots is equivalent to ignoring this fact. Also, you are evaluating your analysis as vague, and I agree because you are drawing conclusions from one or two specific cases and this is far from being an acceptable approach. You are expected to consider the rest of the feedback and propose a clear methodology or procedure to provide evidence about the relationship between volume and stock returns.*
